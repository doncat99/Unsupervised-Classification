...
def main():
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        # Train
        print('Train ...')
        simclr_train(device, train_dataloader, model, criterion, optimizer, epoch)

        # Fill memory bank
        print('Fill memory bank for kNN...')
        fill_memory_bank(device, base_dataloader, model, memory_bank_base)

        # Evaluate (To monitor progress - Not for validation)
        print('Evaluate ...')
        top1 = contrastive_evaluate(device, val_dataloader, model, memory_bank_base)
    ...
    fill_memory_bank(device, val_dataloader, model, memory_bank_val)
    topk = 5
    print('Mine the nearest neighbors (Top-%d)' % (topk))
