...
def main():
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        predictions, features = get_predictions(device, config, dataloader, model, return_features=True)
        clustering_stats = hungarian_evaluate(device, head, predictions, dataset.classes,
