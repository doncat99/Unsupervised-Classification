...
def main():
    ...
    criterion = get_criterion(device, p)
    criterion.to(device)
    print(criterion)
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        # Train
        print('Train ...')
        scan_train(device, train_dataloader, model, criterion, optimizer, epoch, p['update_cluster_head_only'])

        # Evaluate
        print('Make prediction on validation set ...')
        predictions = get_predictions(device, p, val_dataloader, model)
        ...
        clustering_stats = hungarian_evaluate(device, lowest_loss_head, predictions,
    ...
    predictions = get_predictions(device, p, val_dataloader, model)
    clustering_stats = hungarian_evaluate(device, model_checkpoint['head'], predictions,
